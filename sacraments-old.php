...
$totalRows_addsecondconfirmation = mysql_num_rows($addsecondconfirmation);
	...
	if((isset($_GET['second_confirmationid']) && ($_GET['second_confirmationid']!==""))){
mysql_select_db($database_church, $church);
$query_editsecondconfirmation = "SELECT * FROM second_confirmation sc WHERE sc.second_confirmationid=".$_GET['second_confirmationid']."";;
...
mysql_select_db($database_church, $church);
$query_viewsecondconfirmation = "SELECT * FROM second_confirmation s  LEFT JOIN leader l ON l.leaderid=s.leaderid 
...
<ul>
		...
		<li><a href="#tabs-2">Confirmation</a></li>
        <li><a href="#tabs-3">Holy Matrimony</a></li>
        <li><a href="#tabs-4">Deceased</a></li>
...
$totalRows_viewleaderbaptism = mysql_num_rows($viewleaderbaptism);
	...
	?>
      <tr>
        ...
        <td><?php echo $row_viewbaptism['date_baptised']; ?>&nbsp; <input type="text" name="baptismcert_no" value="<?php echo htmlentities($row_editbaptism['baptismcert_no'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
...
?>
  ...
  <table border="1" align="center">
    ...
    <?php do { $numcon++;
/*mysql_select_db($database_church, $church);
$query_confirmationleader= "SELECT * FROM second_confirmation s LEFT JOIN leader l ON s.leaderid=l.leaderid INNER JOIN member_details md ON md.memberid=l.memberid WHERE s.leaderid=". $row_viewsecondconfirmation['leaderid']." ";
...
$query_addmember = "SELECT *  FROM  member_details m
 INNER JOIN gender g ON g.genderid=m.genderid LEFT JOIN marital_status  s ON m.statusid=s.statusid WHERE g.genderid='1'" ;
$addmember = mysql_query($query_addmember, $church) or die(mysql_error());
$row_addmember = mysql_fetch_assoc($addmember);
$totalRows_addmember = mysql_num_rows($addmember);
 ...
 <form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" class="mainbodyview">
   <table align="center">
     <tr valign="baseline">
      ...
      <?php  mysql_select_db($database_church, $church);
$query_addmember = "SELECT  *  member_details m";
$query_limit_addmember = sprintf("%s LIMIT %d, %d", $query_addmember, $startRow_addmember, $maxRows_addmember);
$addmember = mysql_query($query_limit_addmember, $church) or die(mysql_error());
$row_addmember = mysql_fetch_assoc($addmember); 
?>    
